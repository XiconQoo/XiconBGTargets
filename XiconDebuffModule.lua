function GetXiconBGDebuffModule()

    local LIB_NAME = "XiconDebuffModule_BG"
    ...
    local XiconDebuffLib_BG = CreateFrame("Frame", LIB_NAME, UIParent)
    ...
    local function addIcons(dstName, namePlate)
        ...
        for i = 1, #trackedUnitNames[dstName] do
            ...
            trackedUnitNames[dstName][i]:SetFrameStrata(strata)
            ...
            if i == 1 then
                trackedUnitNames[dstName][i]:SetPoint(XiconDebuffSV_local["point"], namePlate, XiconDebuffSV_local["relativePoint"], XiconDebuffSV_local["xOffset"], XiconDebuffSV_local["yOffset"])
            else
                trackedUnitNames[dstName][i]:SetPoint(XiconDebuffSV_local["point"], trackedUnitNames[dstName][i - 1], -size - 2, 0)
            end
            ...
            for i = 1, 40 do
                ...
                if trackedCC[spellName] and timeLeft then
                    ...
                    if timeLeft > 0.5 then
                        XiconDebuffLib_BG:SendMessage(string.format("SPELL_AURA_REFRESH:%s,%s,%s,%s,%s,%s,%s", trackedCC[spellName].id, spellName, unitName, unitGUID, duration, timeLeft, "enemy"))
                    end
                end
            end
        end
    ...
    function events:COMBAT_LOG_EVENT_UNFILTERED(...)
        ...
        if dstIsEnemy and trackedCC[spellName] then
            if eventType == "SPELL_AURA_APPLIED" or eventType == "SPELL_AURA_REFRESH" then
                ...
                XiconDebuffLib_BG:addOrRefreshDebuff(name, dstGUID, spellID)
                XiconDebuffLib_BG:SendMessage(string.format(eventType .. ":%s,%s,%s,%s,%s,%s,%s", spellID, spellName, name, dstGUID, "nil", "nil", "enemy"))
        ...
        if srcIsEnemy and eventType == "UNIT_DIED" then
            ...
            if trackedUnitNames[name .. dstGUID] then
                ...
                XiconDebuffLib_BG:SendMessage(string.format(eventType .. ":%s,%s,%s,%s,%s,%s,%s", "nil", "nil", name, dstGUID, "nil", "nil", "enemy"))
            end
        end
    end
